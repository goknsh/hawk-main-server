                ...
                if (password_verify($pass, $dbPass)) {
                    ...
                    foreach($sitesArr as $site) {
                        ...
                        $siteArr2 = array(
                            ...
                            'us-uptime' => floatval($GLOBALS['conn']->query("SELECT `us-uptime` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'ie-uptime' => floatval($GLOBALS['conn']->query("SELECT `ie-uptime` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'us-uptime-mn' => floatval($GLOBALS['conn']->query("SELECT `us-uptime-mn` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'ie-uptime-mn' => floatval($GLOBALS['conn']->query("SELECT `ie-uptime-mn` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'us-uptime-wk' => floatval($GLOBALS['conn']->query("SELECT `us-uptime-wk` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'ie-uptime-wk' => floatval($GLOBALS['conn']->query("SELECT `ie-uptime-wk` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn()),
                            'us-ssl-exp' => $GLOBALS['conn']->query("SELECT `us-ssl-exp` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn(),
                            'ie-ssl-exp' => $GLOBALS['conn']->query("SELECT `ie-ssl-exp` from `sites` WHERE site='$site' AND email='$email'")->fetchColumn(),
                        );
